...
Observability context for recording custom metrics and span data. See [Telemetry](/docs/telemetry) for more details.

```ts
handler: async (ctx) => {
  ctx.observe.recordMetric('custom.counter', 1)
  ctx.observe.addSpanAttribute('customKey', 'value')
  ctx.observe.addSpanEvent('custom.event')
}
```
