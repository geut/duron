...
- `concurrency_step_limit` - Step concurrency limit for this job
- `client_id` - ID of the Duron instance that owns the job (nullable)
...
- `parent_step_id` - Parent step ID for nested steps (nullable)
- `parallel` - Whether this step runs in parallel with siblings (column name: `branch`)
- `name` - Step name
- `output` - JSONB output data (nullable)
- `error` - JSONB error data (nullable)
...
- `timeout_ms` - Timeout in milliseconds
- `expires_at` - Expiration timestamp (nullable)
- `retries_limit` - Maximum number of retries
- `retries_count` - Current retry count
- `delayed_ms` - Delay before next retry in milliseconds (nullable)
- `history_failed_attempts` - JSONB record of previous failed attempts
- `created_at` - Timestamp
...
Stores OpenTelemetry spans for observability. Only used when `telemetry.local` is enabled.

- `id` - Bigserial primary key
- `trace_id` - OpenTelemetry trace ID (32-char hex)
- `span_id` - OpenTelemetry span ID (16-char hex)
- `parent_span_id` - Parent span ID (nullable, null for root spans)
- `job_id` - Foreign key to jobs table (nullable)
- `step_id` - Foreign key to job_steps table (nullable)
- `name` - Span name
...
- `end_time_unix_nano` - End time in nanoseconds since epoch (nullable)
- `status_code` - Status code (0=UNSET, 1=OK, 2=ERROR)
- `status_message` - Status message (nullable)
