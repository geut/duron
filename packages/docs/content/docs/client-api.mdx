...
**Optional.** Interval in milliseconds between periodic job recovery runs. Defaults to `60000` (1 minute). Set to `0` to disable periodic recovery (only runs on startup).

Recovery checks for:

- **Expired jobs**: Jobs that exceeded their timeout are archived as failed
- **Stuck jobs**: Jobs from unresponsive processes are reset to `created` status

```ts
{
  recoverJobsInterval: 30_000, // Run recovery every 30 seconds
}
```

### `heartbeatInterval`

**Optional.** Interval in milliseconds between heartbeat updates. Each instance periodically upserts a liveness record in the `clients` table. Defaults to `5000` (5 seconds).
...
**Optional.** Milliseconds after which a client without a heartbeat is considered dead. A dead client's active jobs are recovered (requeued or failed if expired). Should be at least 3x `heartbeatInterval`. Defaults to `15000` (15 seconds).

```ts
{
  heartbeatTimeout: 15_000, // 15 seconds
}
```
...
```ts
// With timeout
const result = await client.runActionAndWait(
  'processOrder',
  { orderId: '123' },
  {
