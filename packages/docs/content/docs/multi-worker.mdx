...
const client = duron({
  ...
  processTimeout: 500, // 0.5 seconds (default)
...
1. **Periodic recovery**: Recovery runs automatically based on `recoverJobsInterval` (default: every 60 seconds)
2. **On startup**: Each worker checks for stuck jobs (jobs marked as active but owned by other processes)
3. **Ping check**: Before recovering jobs, workers ping the owning process to verify it's still running
4. **Timeout**: If a process doesn't respond within `processTimeout`, its jobs are recovered
5. **Recovery behavior**:
...
- **Very short timeout (500ms)**: Default, good for most use cases with fast ping responses
- **Short timeout (1-2 seconds)**: For fast deployments and quick recovery
- **Medium timeout (5+ seconds)**: For slow deployments or high-latency networks
