...
const app = createServer({
  ...
  login: {
    ...
    expirationTime: '1h', // Optional, defaults to '1h'
  },
})
...
**Optional.** Expiration time for the access JWT token. Defaults to `'1h'`. Accepts any valid time string format (e.g., `'1h'`, `'7d'`, `'30m'`).

```ts
{
  login: {
    jwtSecret: process.env.JWT_SECRET || 'your-secret-key',
    expirationTime: '30m', // Access token expires in 30 minutes
  },
}
```

#### `login.refreshTokenExpirationTime`

**Optional.** Expiration time for the refresh token. Defaults to `'7d'`. Accepts any valid time string format.
...
```ts
{
  login: {
    jwtSecret: process.env.JWT_SECRET || 'your-secret-key',
    expirationTime: '1h',
    refreshTokenExpirationTime: '30d', // Refresh token expires in 30 days
  },
}
```
...
**Optional.** Enable spans endpoints (`/jobs/:id/spans`, `/steps/:id/spans`). Only works when client is configured with `telemetry.local` enabled. When `true`, enables the dashboard to show spans buttons. Defaults to auto-detected from `client.spansEnabled`.
...
- `fDescription` - Filter by job description (fuzzy match)
...
- `sort` - Sort string format: `"field:asc,field:desc"` (fields: `createdAt`, `startedAt`, `finishedAt`, `status`, `actionName`, `expiresAt`, `duration`, `description`)
...
#### `POST /jobs/:id/time-travel`

Time travel a job to restart from a specific step. The job must be in completed, failed, or cancelled status.
...
{
  "success": true,
  "message": "Job uuid has been time traveled to step step-uuid"
}
```
...
{
  "success": true,
  "message": "Job uuid has been deleted"
}
```

**Status Codes:**
- `200` - Success
- `404` - Job not found or cannot be deleted (active jobs)

#### `DELETE /jobs`

Delete multiple jobs using the same query parameters as `GET /jobs`. Active jobs are excluded from deletion.
...
Spans endpoints are only available when `spansEnabled` is true (auto-detected when `telemetry.local` is enabled on the client).
...
**Query Parameters:**
- `fName` - Filter by span name (string or comma-separated)
- `fKind` - Filter by span kind (0=INTERNAL, 1=SERVER, 2=CLIENT, 3=PRODUCER, 4=CONSUMER)
- `fTraceId` - Filter by trace ID
- `fAttributesFilter` - JSONB filter on attributes (JSON string)
- `sort` - Sort string format: `"field:asc"` or `"field:desc"`
  ...
  "data": [
    {
      ...
      "statusCode": 1,
      "statusMessage": null,
      "attributes": { "duron.job.id": "job-uuid" },
      "events": []
    }
  ],
  "total": 1
...
**Query Parameters:** Same as `GET /jobs/:id/spans`

**Response:** Same format as `GET /jobs/:id/spans`

### Config

#### `GET /config`

Get server configuration. This endpoint does not require authentication.
